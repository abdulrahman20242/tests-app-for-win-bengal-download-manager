...
def test_dependency_tools_standalone_yt_dlp_url():
    """Verify that DEPENDENCY_TOOLS uses standalone yt-dlp binary builds, on
    whichever platform this actually runs on (see core/media_downloader.py's
    IS_WINDOWS/IS_ARM-conditional URL selection)."""
    from core.media_downloader import DEPENDENCY_TOOLS, IS_WINDOWS, IS_ARM
    url = DEPENDENCY_TOOLS["yt-dlp"]["url"]
    if IS_WINDOWS:
        assert url.endswith("yt-dlp_arm64.exe" if IS_ARM else "yt-dlp.exe")
    else:
