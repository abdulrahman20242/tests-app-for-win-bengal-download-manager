...
def test_executable_command_detection(monkeypatch, tmp_path):
    """Cross-platform: get_executable_command()'s AppImage/Flatpak/Snap/frozen
    detection doesn't depend on OS-specific autostart mechanisms, so this runs
    unconditionally on every platform."""
    from core.utils import get_executable_command
...
@pytest.mark.skipif(sys.platform != "linux", reason="XDG .desktop autostart file is Linux-specific; Windows uses the registry Run key (see test_autostart_windows_registry)")
def test_autostart_desktop_file_management(monkeypatch, tmp_path):
    from core.utils import set_autostart_enabled, is_autostart_enabled
...
@pytest.mark.skipif(sys.platform != "win32", reason="Exercises the HKCU Run-key autostart implementation, which only exists on Windows")
def test_autostart_windows_registry(monkeypatch):
    """Windows counterpart to test_autostart_desktop_file_management: verifies
    set_autostart_enabled()/is_autostart_enabled() against a real (but isolated,
    disposable) HKCU Run value rather than the Linux .desktop file path. This
    previously had zero coverage -- the whole registry branch was untested."""
    import winreg
    from core.utils import (
        set_autostart_enabled, is_autostart_enabled,
        _WINDOWS_AUTOSTART_KEY, _WINDOWS_AUTOSTART_VALUE_NAME,
    )
    ...
    def _cleanup():
        try:
            ...
            pass
    _cleanup()  # in case a previous failed run left a stale value behind

    try:
        assert is_autostart_enabled() is False

        assert set_autostart_enabled(True, start_minimized=True) is True
        assert is_autostart_enabled() is True
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, _WINDOWS_AUTOSTART_KEY, 0, winreg.KEY_READ) as key:
            value, _ = winreg.QueryValueEx(key, _WINDOWS_AUTOSTART_VALUE_NAME)
        assert "--minimized" in value

        assert set_autostart_enabled(False) is True
        assert is_autostart_enabled() is False
    finally:
        _cleanup()
...
def test_get_clean_env(monkeypatch):
    ...
    # NOTE: os.pathsep, not a hardcoded ":" -- get_clean_env() prepends extra_paths
    # using os.pathsep specifically so this works on Windows (";") as well as
    # Linux/macOS (":"). A hardcoded ":" here would make this test fail on Windows
    # for the wrong reason: not because the behavior is Linux-only, but because
    # the assertion was checking for the bug get_clean_env() was fixed to avoid.
    assert clean["PATH"].startswith(f"/custom/bin{os.pathsep}")
...
@pytest.mark.parametrize("label,url,expected", [
    (
        "youtube_mix_radio",
        "https://www.youtube.com/watch?v=obBcRhl57Zg&list=RDobBcRhl57Zg&start_radio=1&pp=ygUGamFobnZpoAcB",
        "https://www.youtube.com/watch?v=obBcRhl57Zg",
    ),
    (
        "youtube_real_playlist_preserved",
        "https://www.youtube.com/playlist?list=PL1234567890abcdef",
        "https://www.youtube.com/playlist?list=PL1234567890abcdef",
    ),
    (
        "youtube_shorts_tracking",
        "https://www.youtube.com/shorts/abcdef12345?si=sample_si_param&feature=share",
        "https://www.youtube.com/shorts/abcdef12345",
    ),
    (
        "youtu_be_short_url",
        "https://youtu.be/obBcRhl57Zg?si=track123&feature=shared",
        "https://youtu.be/obBcRhl57Zg",
    ),
    (
        "tiktok_tracking",
        "https://www.tiktok.com/@creator/video/71234567890?is_from_webapp=1&sender_device=pc",
        "https://www.tiktok.com/@creator/video/71234567890",
    ),
    (
        "plain_url_without_tracking",
        # Was a dangling comment with no assertion at all -- the regression guard
        # for "sanitize_media_url() doesn't need to change anything" was never
        # actually checked.
        "https://www.youtube.com/watch?v=obBcRhl57Zg",
        "https://www.youtube.com/watch?v=obBcRhl57Zg",
    ),
])
def test_sanitize_media_url(label, url, expected):
    from core.utils import sanitize_media_url
    assert sanitize_media_url(url) == expected
...
@pytest.mark.skipif(sys.platform != "linux", reason="Snap confinement (SNAP_REAL_HOME/SNAP_USER_DATA) is a Linux-only packaging concept with no Windows equivalent")
