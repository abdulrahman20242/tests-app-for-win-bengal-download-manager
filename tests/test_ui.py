...
def test_header_highlight_sections_disabled_and_max_conn_default(qapp):
    ...
    opt_dlg.close()
    window.close()
def test_main_window_start_minimized_flag_sets_tray_state_on_init(qapp):
    """Real replacement for the old test, which only asserted Python's `in`
    operator on two hardcoded literal lists and exercised zero application
    code (it would pass identically for any app, or none at all). Verifies
    MainWindow's actual __init__-time start_minimized handling
    (src/ui/main_window.py:162-167), which is what main.py's --minimized argv
    flag ultimately drives (see main.py:235-236)."""
    from main import MainWindow

    MainWindow.start_minimized = True
    try:
        window = MainWindow(start_ipc=False)
        # _is_in_tray is set synchronously in __init__ when start_minimized is
        # truthy; the actual hide()/update_tray_action() calls are deferred via
        # QTimer.singleShot(0, ...), so this is the directly-observable half of
        # the same code path without needing to pump the event loop.
        assert window._is_in_tray is True
    finally:
        del MainWindow.start_minimized
        window.close()

    window_normal = MainWindow(start_ipc=False)
    try:
        assert getattr(window_normal, "_is_in_tray", False) is False
    finally:
        window_normal.close()
...
def test_options_dialog_height_matches_main_window(qapp):
    ...
    # options.py:174-180: target_height defaults to 520, but becomes the main
    # window's height whenever that's > 200px -- 680 here, so this is the real
    # code path the test's name and docstring claim to verify.
...
@pytest.mark.skipif(sys.platform != "linux", reason="Verifies the Linux nautilus/DBus file-manager fallback specifically; doesn't mock platform.system(), so on Windows show_in_folder() correctly takes the real Windows branch instead")
