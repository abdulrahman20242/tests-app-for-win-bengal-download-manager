...
def test_get_unique_filepath_with_existing_names_and_paths():
    ...
    # Built with os.path.join rather than a hardcoded "/tmp/virtual/..." literal:
    # get_unique_filepath() reconstructs its result via os.path.join(dirname,
    # filename), which inserts a backslash on Windows even when the input used
    # forward slashes -- a hardcoded all-forward-slash expectation would fail
    # there for a path-formatting reason unrelated to the numbering logic this
    # test actually exists to verify.
    base_dir = os.path.join(os.sep, "tmp", "virtual")

    # When file does NOT exist on disk, but name is in existing_names
    res1 = get_unique_filepath(os.path.join(base_dir, "testfile.txt"), existing_names={"testfile.txt"})
    assert res1 == os.path.join(base_dir, "testfile (1).txt")

    # Incremental numbering (1) -> (2)
    res2 = get_unique_filepath(os.path.join(base_dir, "testfile.txt"), existing_names={"testfile.txt", "testfile (1).txt"})
    assert res2 == os.path.join(base_dir, "testfile (2).txt")

    # Base already has number
    res3 = get_unique_filepath(os.path.join(base_dir, "testfile (1).txt"), existing_names={"testfile (1).txt"})
    assert res3 == os.path.join(base_dir, "testfile (2).txt")
